...
[1] [P2] DetailPage left rail renders permanent skeleton loading state on fetch error
    ...
    Suggested fix: Check `detail.state.status === 'loading'` when rendering rail skeletons, and render an empty or muted state in the rail when `detail.state.status === 'error'` or `'unauthorized'`.
