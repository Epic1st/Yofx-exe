...
[1] [P1] UserIdentity.VerifyEmail bypasses lock and recovery states when email is unverified
    ...
    Suggested fix: Restrict `VerifyEmail` to only transition `SecurityState` from `UserSecurityState.Invited` to `UserSecurityState.Active`, rejecting verification or throwing a `DomainException` if `SecurityState` is `Locked` or `RecoveryRequired`.
