...
[1] [P2] LinkAccountModal test suite never exercises submission failure or rejection branches
    ...
    Suggested fix: Add test cases in `LinkAccountModal.test.tsx` verifying that `onSubmit` resolving `false` renders the retry error message while preserving the dialog state, and `onSubmit` rejecting with an `Error` renders the thrown error message.
