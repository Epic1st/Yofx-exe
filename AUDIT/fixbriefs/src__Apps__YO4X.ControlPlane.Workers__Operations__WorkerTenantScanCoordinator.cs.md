...
[1] [P3] WorkerTenantScanLease lifecycle SemaphoreSlim leaked on every cycle disposal
    ...
    Suggested fix: In `WorkerTenantScanLease.DisposeAsync()`, dispose `lifecycleGate` after releasing it, or avoid allocating a per-lease `SemaphoreSlim` by using a lightweight non-blocking synchronisation mechanism.
