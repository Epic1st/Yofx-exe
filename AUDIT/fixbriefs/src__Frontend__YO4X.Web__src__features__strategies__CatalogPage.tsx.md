...
[1] [P2] CatalogPage facet filter chips unmount during in-flight queries and disappear on error
    ...
    Suggested fix: Retain the last non-null `categories` and `symbols` arrays (e.g. via local state or a ref) so chips remain mounted and clickable during loading and error states.
