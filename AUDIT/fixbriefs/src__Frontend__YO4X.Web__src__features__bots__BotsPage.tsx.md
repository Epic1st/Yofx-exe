...
[1] [P2] Single `pendingBotId` state permits concurrent double-click and state desynchronization across bot rows
    ...
    Suggested fix: Maintain a `Set<string>` of in-flight bot IDs so each row independently tracks pending state.
