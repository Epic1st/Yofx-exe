...
[1] [P2] `ClientCertificateFilter` compares local `DateTime` certificate expiration timestamps against `DateTimeOffset.UtcNow`
    ...
    Suggested fix: Use `certificate.GetNotBefore()` and `certificate.GetNotAfter()` (or `certificate.NotBefore.ToUniversalTime()` and `certificate.NotAfter.ToUniversalTime()`).
