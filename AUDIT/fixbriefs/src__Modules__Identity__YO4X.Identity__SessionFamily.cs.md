...
[1] [P2] SessionFamily.Rotate prioritizes expiration over compromised token detection
    ...
    Suggested fix: Check `_invalidatedTokenHashes.Contains(presentedTokenHash)` before checking expiration in `Rotate()`, transitioning the state to `Compromised` whenever an invalidated token hash is presented.
