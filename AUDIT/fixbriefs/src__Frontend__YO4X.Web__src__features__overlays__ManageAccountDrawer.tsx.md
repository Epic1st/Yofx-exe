...
[1] [P2] Credential state error in ManageAccountDrawer displays perpetual 'Loading…' with no error or retry state
    ...
    Suggested fix: Check `credential.state.status === 'error'` in `figures` and display `'Failed to load'` alongside a retry button.
