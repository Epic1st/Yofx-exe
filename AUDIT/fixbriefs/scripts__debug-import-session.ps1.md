...
[1] [P2] Plaintext bearer token persisted in `debug-import-session.ps1` without `finally` cleanup guarantee
    ...
    Suggested fix: Wrap the curl execution and file deletion in a `try ... finally` block to guarantee `$body` and `$cfg` are removed upon exit.
