...
[1] [P2] `.detail-meta__verified` uses graphical `--color-positive` token instead of typography token
    ...
    Suggested fix: Change `color: var(--color-positive);` to `color: var(--color-positive-text);` in `features/strategies/detail.css:306`.
