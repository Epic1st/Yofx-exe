...
[1] [P2] `Mql5InventoryFormatter` Over-Reports Finding File Counts on Multiple Findings Per File
    ...
    Suggested fix: Calculate distinct file count across the group (e.g. counting distinct `file.RelativePath` containing the finding) or rename the Markdown column header to `Findings` / `Occurrences`.
