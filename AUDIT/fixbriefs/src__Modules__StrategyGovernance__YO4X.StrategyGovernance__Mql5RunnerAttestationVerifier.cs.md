...
[1] [P2] Runner Public Keys Lack Lifecycle Validity Windows and Expiry Enforcement
    ...
    Suggested fix: Associate validity time intervals (`ValidFromUtc`, `ValidUntilUtc`) with runner public keys and verify that `descriptor.CompletedAtUtc` falls within the key's active validity window.
