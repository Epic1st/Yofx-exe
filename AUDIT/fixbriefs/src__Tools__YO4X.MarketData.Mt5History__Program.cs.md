...
[1] [P2] Single-bar history downloads are rejected by period validation
    ...
    Suggested fix: If `bars.Count < 2`, skip the inter-bar spacing validation and return `(int)period` so single-bar downloads are written successfully.
