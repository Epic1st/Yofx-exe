...
[1] [P2] `Test-PostgresIntegration.ps1` fallback hardcodes local author user profile path
    ...
    Suggested fix: Replace the hardcoded path with `Join-Path $workspaceRoot ".tools\dotnet-sdk-10.0.400\dotnet.exe"` or dynamically construct the path using `Join-Path $env:LOCALAPPDATA "YO4X\dotnet\dotnet.exe"`.
