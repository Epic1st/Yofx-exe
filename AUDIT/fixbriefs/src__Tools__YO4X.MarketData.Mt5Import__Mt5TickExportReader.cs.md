...
[1] [P1] `Mt5TickExportReader` applies a single fixed UTC offset across historical tick data, ignoring DST
    ...
    Suggested fix: Support timezone identifiers (e.g. `Europe/Athens` / `EET`) with historical DST transition rules in `Mt5TickExportReader` to dynamically resolve the UTC offset for each tick timestamp.
