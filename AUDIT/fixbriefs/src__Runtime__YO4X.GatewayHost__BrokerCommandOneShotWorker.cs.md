...
[1] [P2] BrokerCommandOneShotWorker catches host shutdown cancellation and marks runtime status Failed
    ...
    Suggested fix: Add `catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)` prior to the general catch block to allow clean shutdown without marking the status as `Failed`.
