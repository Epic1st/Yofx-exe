...
[1] [P1] String-to-number helpers fail on numeric prefixes with trailing non-numeric characters
    ...
    Suggested fix: Replace strict `long.TryParse` / `double.TryParse` with leading numeric prefix parsers that match MQL5 `StringToInteger` and `StringToDouble` semantics.
