...
THE ONLY FILE YOU MAY MODIFY:
  src/Runtime/YO4X.Trading.Abstractions/GatewayModels.cs

Read that file completely first. You may read any other file for context, but you must not edit any other file, create files, delete files, or run commands.

FINDINGS TO FIX (3):

[1] [P1] Default enum values across trading and strategy domain map to active execution members (`Buy`, `Market`, `Place`, `Accepted`) rather than `Unknown`
    ...
    Suggested fix: Introduce explicit `Unknown = 0` / `Unspecified = 0` members as the default value for each enum, starting operational members at index 1.

[2] [P2] `GatewayOperationResult<T>` permits `IsSuccess = true` with `Value = null`, breaking consumer null-safety assumptions
    ...
    Suggested fix: Add static factory methods `Success(T value, string code)` throwing on null, and `Failure(string code, T? value = null)`.

[3] [P3] `BrokerDealSnapshot` omits `PositionId` and `DealEntryType`, preventing reconciliation of position lifecycle and fees
    ...
    Suggested fix: Add `string? PositionId` and `string? DealEntry` to `BrokerDealSnapshot`.
