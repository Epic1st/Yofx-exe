...
[1] [P2] `OutboxSchemaVersion.ValidateStored` silently skips validation for non-v4 message types with malformed versions
    ...
    Suggested fix: Enforce `requiresExactSchemaVersion = true` for all versioned message types matching `yo4x.*.vN` where `resolvedVersion > 1`, and reject non-numeric version values regardless of `required`.
