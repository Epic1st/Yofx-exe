...
[1] [P0] Unimplemented `OrderCalcMargin` in backtesting context breaks standard library position sizing
    ...
    Suggested fix: Implement `OrderCalcMargin` and `OrderCalcProfit` in `Mql5MarketContext` by delegating to `Mql5SymbolSpec.MarginOf` and `Mql5SymbolSpec.ProfitOf`.
