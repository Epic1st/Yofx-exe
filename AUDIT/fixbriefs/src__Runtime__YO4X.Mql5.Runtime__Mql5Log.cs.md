...
[1] [P3] `Mql5LogRecorder.Log` contains redundant branches and decrements `count` on failed dequeue
    ...
    Suggested fix: Remove the redundant `else` branch so `count` is only decremented when `TryDequeue` successfully removes an item.
