...
[1] [P2] Dialog focus trap allows focus to escape outside modal on Shift+Tab from dialog root surface
    ...
    Suggested fix: Update the reverse tab condition in `useDialogBehaviour` to check `if (event.shiftKey && (active === first || active === surface.current)) { event.preventDefault(); last.focus(); }`.
