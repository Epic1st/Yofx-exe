...
[1] [P2] Missing `RuntimeEvidencePostgres` is tolerated during registration but permanently fails the readiness probe
    ...
    Suggested fix: Make `RuntimeEvidencePostgresDatabase` registration mandatory in `TryAddRuntimeControlPostgres` (returning `services` early without registering `PostgresRuntimeControlPlaneApplication` if evidence database configuration fails) so the host falls back to `UnavailableRuntimeControlPlaneApplication`.
