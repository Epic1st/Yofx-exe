...
[1] [P2] `AddYo4xEmergencyAuthentication` omits `IHostEnvironment`, breaking development authority certificate pinning
    ...
    Suggested fix: Add `IHostEnvironment? environment = null` to `AddYo4xEmergencyAuthentication` and forward it into `ConfigureJwt(options, configuration, "Emergency", environment)`.
