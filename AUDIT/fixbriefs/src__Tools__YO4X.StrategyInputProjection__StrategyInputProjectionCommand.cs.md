...
[1] [P2] Unrecognized CLI arguments and typos in `YO4X.StrategyInputProjection` are silently ignored
    ...
    Suggested fix: Validate all tokens in `arguments` against the set of accepted option flags, throwing `ArgumentException` when unknown parameters are supplied.

[2] [P2] `YO4X.StrategyInputProjection` ignores `--help` and executes full projection
    ...
    Suggested fix: Add a check for `--help` or `-h` at the start of `RunAsync` or `ParseOptions` that invokes `WriteUsage()` and exits cleanly with return code 0.

[3] [P3] Generated diagnostic query counts rows globally across all tenants
    ...
    Suggested fix: Add `where tenant_id = <tenant>::uuid` to both subqueries in the diagnostic output block.
