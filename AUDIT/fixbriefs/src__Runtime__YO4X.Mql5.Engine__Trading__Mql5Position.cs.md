...
[1] [P3] `Mql5Position` is an unencapsulated mutable DTO lacking lifecycle state invariants
    ...
    Suggested fix: Add an `IsOpen` boolean property to `Mql5Position` set to `false` on position closure, or clear ticket and pricing fields when volume drops to zero.
