...
[1] [P2] Missing positive check constraint on `simulation.backtests.equity_initial_deposit` permits zero and negative initial capital
    ...
    Suggested fix: Add `check (equity_initial_deposit > 0)` to `simulation.backtests.equity_initial_deposit`.
