//+------------------------------------------------------------------+
//|                                              CMaan Hedging EA.mq5|
//|                                                 CikguMaan, 2026  |
//+------------------------------------------------------------------+
#property copyright "CikguMaan, 2026"
#property link      ""
...
input string InpSymbol                    = "XAUUSD.s"; // Symbol (include suffix if needed eg: XAUUSD.s)
input double InpLots                      = 0.01; // Initial Lot Size
input int    InpDeviationPoints           = 50; // Max Spread Allowed (Points)
input long   InpMagic                     = 260320264; //Magic Number
input bool   InpOnlyM1                    = true; // Use only M1 Timeframe Chart
input bool   InpVerbose                   = true; // Send (print) Logs to Journal

// Re-entry behavior (kept from your original EA)
input group "=== Re-Entry Settings ==="
input int    InpReentryMinDelaySec        = 8; // Cooldown time for new order 
input int    InpReentryForceSec           = 45; // Hard Time Limit for new order
input double InpReentryGapAbs             = 0.45; // Minimum Gap for open new orders
input double InpReentryGapSpreadMult      = 1.25; // Multiplier for Minimum Gap allowed (for high volatility)

// New closing mechanism
input group "=== Exit Mechanism ==="
input double InpCloseTargetPips           = 40.0; //Close at Target Level (Pips)
input double InpCloseRetracePips          = 10.0; //Close at Retrace Level (Pips)
input double InpCloseLevelStepPips        = 10.0; //CLose Level Step Size (Pips) 
input bool   InpCloseAutoPipSize          = true; //Auto PipSize calculation (2 or 3 digits)
input double InpClosePipSize              = 0.10; //Manual PipSize (eg for XAUUSD: 0.10 is 1pip or 10 points)
input bool   InpVerboseCloseDebug         = false; //Journal Tracking (Default "False" to save spams broker)
...
void OnTick()
{
   ...
   // Add a 1-second cooldown between trade attempts to prevent broker server spam
   if(TimeCurrent() > last_trade_attempt) 
   {
      MaintainInventory();
      if(changed) 
         MaintainInventory();
         
      last_trade_attempt = TimeCurrent();
   }
}
...
bool ReentryReady(const ENUM_POSITION_TYPE side)
{
   ...
   if(side == POSITION_TYPE_BUY)
   {
      if(!g_pendingBuy || g_pendingBuyTime <= 0)
         return true;
   ...
   if(!g_pendingSell || g_pendingSellTime <= 0)
      return true;
...
bool ManageCloseMechanism()
{
   ...
   if(sig == CLOSE_SIGNAL_SELL && have_sell)
      return CloseManagedTicket(sell.ticket);

   if(sig == CLOSE_SIGNAL_BUY && have_buy)
