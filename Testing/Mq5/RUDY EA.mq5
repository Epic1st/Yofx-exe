//+------------------------------------------------------------------+
//|  RUDY Price Action Concepts EA (Debugged)                        |
...
int     lastInternalHighIndex = -1, lastInternalLowIndex = -1;
int     lastSwingHighIndex    = -1, lastSwingLowIndex    = -1;
...
int OnInit()
{
    ...
    // DEBUG: Changed from FOK to IOC for better ECN broker compatibility
...
{
    //--- Internal pivots (Scan for most recent pivot) ---
    int currIH_Index, currIL_Index;
    double iH = GetRecentPivotHigh(InternalLookback, currIH_Index);
    double iL = GetRecentPivotLow (InternalLookback, currIL_Index);

    //--- Swing pivots ---
    int currSH_Index, currSL_Index;
    double sH = GetRecentPivotHigh(SwingLookback, currSH_Index);
    double sL = GetRecentPivotLow (SwingLookback, currSL_Index);

    // Update State if we found a NEW pivot (index is smaller/higher in history)
    // Note: In MQL5 arrays/history, index 1 is newest. Smaller index = More Recent.
    if (iH > 0 && (lastInternalHighIndex == -1 || currIH_Index < lastInternalHighIndex))
    {
        lastInternalHigh = iH;
        lastInternalHighIndex = currIH_Index;
    }
    if (iL > 0 && (lastInternalLowIndex == -1 || currIL_Index < lastInternalLowIndex))
    {
        lastInternalLow = iL;
        lastInternalLowIndex = currIL_Index;
    }

    if (sH > 0 && (lastSwingHighIndex == -1 || currSH_Index < lastSwingHighIndex))
    {
        lastSwingHigh = sH;
        lastSwingHighIndex = currSH_Index;
    }
    if (sL > 0 && (lastSwingLowIndex == -1 || currSL_Index < lastSwingLowIndex))
    {
        lastSwingLow = sL;
        lastSwingLowIndex = currSL_Index;
    }
    ...
    if (lastInternalHigh > 0 && close0 > lastInternalHigh)
    {
        ...
        if (draw)
        {
            DrawStructureLine(true, structType, lastInternalHighIndex, lastInternalHigh);
            if (TradeOrderBlocks && FilterPass(structType))
                AddInternalOB(true, lastInternalHighIndex);
        }

        internalTrend = 1;
        lastInternalHigh = 0;
        lastInternalHighIndex = -1; // Reset to find next one
    }
    ...
    if (lastInternalLow > 0 && close0 < lastInternalLow)
    {
        ...
        if (draw)
        {
            DrawStructureLine(false, structType, lastInternalLowIndex, lastInternalLow);
            if (TradeOrderBlocks && FilterPass(structType))
                AddInternalOB(false, lastInternalLowIndex);
        }

        internalTrend = -1;
        lastInternalLow = 0;
        lastInternalLowIndex = -1;
    }
    ...
    if (lastSwingHigh > 0 && close0 > lastSwingHigh)
    {
        ...
        DrawStructureLine(true, "S-" + structType, lastSwingHighIndex, lastSwingHigh);
        swingTrend  = 1;
        lastSwingHigh = 0;
    ...
    if (lastSwingLow > 0 && close0 < lastSwingLow)
    {
        ...
        DrawStructureLine(false, "S-" + structType, lastSwingLowIndex, lastSwingLow);
        swingTrend  = -1;
        lastSwingLow = 0;
        lastSwingLowIndex = -1;
    }
...
//+------------------------------------------------------------------+
//| Detect 3-candle Fair Value Gaps (Corrected Indices)             |
//+------------------------------------------------------------------+
void DetectFVG()
{
    // We use CLOSED bars. 
    // Bar 1 is the last closed bar. Bar 3 is the one before the middle candle (Bar 2).
    ...
    if (l1 > h3)
    {
        ...
        fvg.bull    = true;
        fvg.active  = true;
        fvg.created = iTime(_Symbol, PERIOD_CURRENT, 1);
    ...
    for (int i = 0; i < ArraySize(FVGs); i++)
    {
        ...
        // If Bullish FVG and Price drops below bottom
        if ( FVGs[i].bull  && close0 < FVGs[i].bottom) FVGs[i].active = false;
        // If Bearish FVG and Price rises above top
...
bool OpenBuy(double sl, double tp)
{
    ...
    if (!trade.Buy(LotSize, _Symbol, ask, sl, tp, "RUDY EA - BUY"))
    {
...
bool OpenSell(double sl, double tp)
{
    ...
    if (!trade.Sell(LotSize, _Symbol, bid, sl, tp, "RUDY EA - SELL"))
    {
...
//+------------------------------------------------------------------+
//| Get most recent pivot high within lookback                        |
//+------------------------------------------------------------------+
double GetRecentPivotHigh(int len, int &foundIndex)
{
    foundIndex = -1;
    // Scan from bar 2 (first closed bar with 1 right neighbor) up to lookback
    for (int i = 2; i < len; i++)
    {
        double pivotHigh = iHigh(_Symbol, PERIOD_CURRENT, i);
        bool isPivot = true;
        
        // Check neighbors (1 left, 1 right) - Standard Fractal
        // For strict lookback match to original logic, you could widen this check.
        // Here we use a small window (1 each side) for frequency.
        if (iHigh(_Symbol, PERIOD_CURRENT, i-1) >= pivotHigh ||
            iHigh(_Symbol, PERIOD_CURRENT, i+1) >= pivotHigh)
        {
            isPivot = false;
        }
...
//+------------------------------------------------------------------+
//| Get most recent pivot low within lookback                         |
//+------------------------------------------------------------------+
double GetRecentPivotLow(int len, int &foundIndex)
{
    foundIndex = -1;
    for (int i = 2; i < len; i++)
    {
        double pivotLow = iLow(_Symbol, PERIOD_CURRENT, i);
        bool isPivot = true;

        if (iLow(_Symbol, PERIOD_CURRENT, i-1) <= pivotLow ||
            iLow(_Symbol, PERIOD_CURRENT, i+1) <= pivotLow)
        {
            isPivot = false;
        }
...
//+------------------------------------------------------------------+
void AddInternalOB(bool bull, int pivotIndex)
{
    OBZone ob;
    // OB is usually the candle BEFORE the pivot (Last Opposing Candle)
    // Pivot at i. OB is at i+1 (older).
    int bar = pivotIndex + 1; 
    ...
    ob.bull    = bull;
    ob.active  = true;
    ob.created = iTime(_Symbol, PERIOD_CURRENT, bar);
...
void DrawStructureLine(bool bull, string label, int barIdx, double price)
{
    ...
    int    fromBar = barIdx + 5; 

    datetime time1 = iTime(_Symbol, PERIOD_CURRENT, fromBar);
    datetime time2 = iTime(_Symbol, PERIOD_CURRENT, 0);

    if(time1 == 0) time1 = time2 - PeriodSeconds()*10; // Safety if history missing
    ...
    ObjectCreate(0, lblName, OBJ_TEXT, 0, time2, price);
