#property copyright  "Copyright 2024 - Wim Schrynemakers"
...
// --- ECN safety clamp added for JustMarkets: keeps every SL modification (trailing + break-even)
// at least (broker StopsLevel + FreezeLevel + current spread + 5-point buffer) away from price,
// so tightened stops from the recalibrated presets never get rejected or trigger on spread noise. ---
double ECN_ClampSL(string sym,int posType,double sl)
{
   if(sl<=0.0) return sl;
   ...
   double minDist=MathMax((double)stopsLevel,(double)freezeLevel)*point + spread + 5.0*point;
   if(posType==POSITION_TYPE_BUY)
   {
      double bid=SymbolInfoDouble(sym,SYMBOL_BID);
      if(bid-sl<minDist) sl=NormalizeDouble(bid-minDist,(int)SymbolInfoInteger(sym,SYMBOL_DIGITS));
   }
   else if(posType==POSITION_TYPE_SELL)
   {
      double ask=SymbolInfoDouble(sym,SYMBOL_ASK);
      if(sl-ask<minDist) sl=NormalizeDouble(ask+minDist,(int)SymbolInfoInteger(sym,SYMBOL_DIGITS));
   }
   return sl;
}
...
bool MQL4_OrderModify(long ticket,double price,double sl,double tp,datetime expiration,color arrow_color=clrNONE)
{
   ...
   {
      string __ecnSym=PositionGetString(POSITION_SYMBOL);
      if(sl>0.0) sl=ECN_ClampSL(__ecnSym,(int)PositionGetInteger(POSITION_TYPE),sl);
      req.action=TRADE_ACTION_SLTP; req.position=(ulong)ticket; req.symbol=__ecnSym; req.sl=sl; req.tp=tp;
   }
   else
   {
...
//lizong_37 <<==--------   --------
 ...
 总_113_do_2A8 = 临_do_12 + 490.0 ;
 临_do_12 = AdjustBreakEven + 170.0;
 if ( Randomization>0.0 )
 {
...
//lizong_38 <<==--------   --------
 ...
 总_113_do_2A8 = 临_do_11 + 410.0 ;
 临_do_11 = AdjustBreakEven + 140.0;
 if ( Randomization>0.0 )
 {
   临_do_12 = Randomization * 2.0 * MathRand() / 32768.0 + (0.0 - Randomization);
 }
...
//lizong_39 <<==--------   --------
 ...
 总_113_do_2A8 = 临_do_12 + 330.0 ;
 临_do_12 = AdjustBreakEven + 120.0;
 if ( Randomization>0.0 )
 {
   临_do_13 = Randomization * 2.0 * MathRand() / 32768.0 + (0.0 - Randomization);
 }
...
//lizong_44 <<==--------   --------
 ...
 总_113_do_2A8 = 临_do_11 + 120.0 ;
 临_do_11 = AdjustBreakEven + 40.0;
 if ( Randomization>0.0 )
 {
   临_do_12 = Randomization * 2.0 * MathRand() / 32768.0 + (0.0 - Randomization);
 }
