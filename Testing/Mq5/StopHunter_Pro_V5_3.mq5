//+------------------------------------------------------------------+
//|                                     StopHunter_Pro_V5_3.mq5      |
//|         Instant BE + Open/Close Pick Points + Color Panel        |
//|      Pendings at Candle Body Top/Bottom (Open or Close)          |
//+------------------------------------------------------------------+
#property copyright "StopHunter Pro V5.3"
#property version   "5.30"
#property strict
#property description "INSTANT BE: SL moves to entry price at ANY profit"
#property description "BuyStop at candle body TOP (higher of Open/Close)"
#property description "SellStop at candle body BOTTOM (lower of Open/Close)"
#property description "Colored dashboard with clear visual status"
...
//+------------------------------------------------------------------+
//| DYNAMIC BREAKEVEN + TRAILING + TP HUNT                            |
//+------------------------------------------------------------------+
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      // ═══════════════════════════════════════════════════════════════
      // === INSTANT BE: Move SL to entry IMMEDIATELY when ANY profit ===
      // ═══════════════════════════════════════════════════════════════
      if(InpInstantBE && pips > 0.5)  // More than 0.5 pip profit = instant BE
        {
         if(type == POSITION_TYPE_BUY)
           {
            double beSL = N(op + 1 * _Point);  // Entry + 1 point (tiny lock)
            if(cSL < beSL)
              {
         ...
         // Don't continue - let trailing and hunt still work on top
        }

      // === TP HUNT MODE (very tight trail when deep in profit) ===
...
int OnInit()
  {
   ...
   Print("═══════════════════════════════════════════════════════");
   Print("   STOPHUNTER PRO V5.3 - OPEN/CLOSE PICK POINTS");
   Print("═══════════════════════════════════════════════════════");
...
void Engine()
  {
   ...
   // Grid complete
   if(g_gridActive && pend == 0 && pos == 0)
     { g_gridActive = false; g_gridCloseTime = TimeCurrent(); }
...
void Panel()
  {
   ...
   // Main background
   CreateBox("BG", x, y, PANEL_W, LINE_H * 28, cBG, cBorder);

   y += 5;

   // === HEADER ===
   CreateLabel("T1", x+10, y, "═══ STOPHUNTER PRO V5.3 ═══", cTitle, 10); y += LINE_H + 2;
   CreateLabel("T2", x+10, y, "  OPEN/CLOSE PICK + INSTANT BE", cCyan, 9); y += LINE_H + 5;
   ...
   y += LINE_H;
   CreateLabel("Sp", x+10, y+3, "Spread: " + IntegerToString(sp), sp < 30 ? cGreen : cOrange);
   ...
   CreateLabel("Tick", x+100, y+3, "Tick: " + tickStr, tickClr);

   // Tick flow counter
   color tfClr = (g_ticksUp >= InpTickConfirm || g_ticksDn >= InpTickConfirm) ? cGreen : cLabel;
   CreateLabel("TF", x+180, y+3, "▲" + IntegerToString(g_ticksUp) + " ▼" + IntegerToString(g_ticksDn), tfClr);
   y += LINE_H + 8;
   ...
   // Instant BE indicator
   CreateLabel("IBE", x+10, y, InpInstantBE ? "⚡ INSTANT BE: ON" : "INSTANT BE: OFF", InpInstantBE ? cGreen : cLabel);
