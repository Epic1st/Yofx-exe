...
//|    Trading logic identical to v1 — only visuals were added.      |
//+------------------------------------------------------------------+
...
void BuildDashboard()
{
   ...
   MakeLabel(BRT+"VER",  PANEL_X+PANEL_W-55, y+10, "v2.0", FONT_BODY, 7, CLR_DIMMED);
   y += HDR_H + 2;
...
int    GetBarIndex()       { return (int)SeriesInfoInteger(_Symbol, PERIOD_CURRENT, SERIES_BARS_COUNT) - 1; }
...
bool DetectReversalAgainstBull(double atr_val)
{
   ...
   if(Use_MomentumFlip)
   {
       double ef0 = GetEMAFast(0), es0 = GetEMASlow(0);
       double ef1 = GetEMAFast(1), es1 = GetEMASlow(1);
       ema_flip = (ef1 >= es1) && (ef0 < es0);
   }
...
bool DetectReversalAgainstBear(double atr_val)
{
   ...
   double c2 = GetClose(2), o2 = GetOpen(2);
   double h1 = GetHigh(1);
   double l0 = GetLow(0),  l1 = GetLow(1);
...
void OnTick()
{
   ...
   int current_bar_idx = iBarShift(_Symbol, PERIOD_CURRENT, current_bar);
   ...
   if(bull_waiting && bull_neck > 0 && trend_ok_buy)
   {
       ...
       if(in_zone && bouncing && bars_since <= Buy_RetestBars)
       {
           if(CountPositions() < MaxPositions)
           {
               ...
               double tp      = entry + atr * Buy_TP_Mult * 2.0;
   ...
   if(bear_waiting && bear_neck > 0 && trend_ok_sell)
   {
       ...
       if(in_zone && bouncing && bars_since <= Sell_RetestBars)
       {
           if(CountPositions() < MaxPositions)
           {
               ...
               double tp_price = entry - atr * Sell_TP_Mult * 2.0;
