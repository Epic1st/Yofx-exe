...
// --- REPLACED: Old time-based delays are gone. 
// --- NEW: True pure price action retracement for Hedge Re-entry
input double InpReentryRetracePips        = 10.0; 
...
// New trackers to hold the extreme point while unhedged
double   g_pendingBuy_LowestAsk = 0.0;
double   g_pendingSell_HighestBid = 0.0;
...
void ResetRuntimeState()
{
   g_pendingBuy = false;
   g_pendingSell = false;
   g_pendingBuy_LowestAsk = 0.0;
   g_pendingSell_HighestBid = 0.0;
...
void RegisterPendingReentry(const ENUM_POSITION_TYPE closed_side)
{
   // As soon as a position closes, initialize tracking using current market price
   double bid = 0.0, ask = 0.0;
   GetBidAsk(bid, ask);

   if(closed_side == POSITION_TYPE_BUY)
   {
      // BUY was just closed -> Active Trade is SELL. Price trend is DOWN.
      // We start tracking the lowest ASK to measure retracement for re-entry.
      g_pendingBuy = true;
      g_pendingBuy_LowestAsk = ask; 
   }
   else if(closed_side == POSITION_TYPE_SELL)
   {
      // SELL was just closed -> Active Trade is BUY. Price trend is UP.
      // We start tracking the highest BID to measure retracement for re-entry.
      g_pendingSell = true;
      g_pendingSell_HighestBid = bid;
   }
}
...
bool OpenSide(const ENUM_POSITION_TYPE side)
{
   ...
   // Reset tracking state after a successful re-entry
   if(side == POSITION_TYPE_BUY)
   {
      g_pendingBuy = false;
      g_pendingBuy_LowestAsk = 0.0;
...
bool CloseManagedTicket(const ulong ticket)
{
   ...
   // Register the pending state to track re-entry
   RegisterPendingReentry(side);
...
bool ReentryReady(const ENUM_POSITION_TYPE sideToOpen)
{
   double bid = 0.0, ask = 0.0;
   if(!GetBidAsk(bid, ask)) return false;

   double pip_price = EffectivePipSize();
   double retrace_points = InpReentryRetracePips * pip_price; // 10 pips default
   double eps = PriceEpsilon(); // Avoid floating point inaccuracies 

   if(sideToOpen == POSITION_TYPE_BUY)
   {
      if(!g_pendingBuy) return true;

      // Active Trade is SELL. It profits as price drops. 
      // Track how low the Ask pushes downward.
      if(g_pendingBuy_LowestAsk <= 0.0 || ask < g_pendingBuy_LowestAsk)
         g_pendingBuy_LowestAsk = ask;

      // Rule: Re-enter BUY when price retraces UP by exactly 10 pips from lowest point
      if(ask >= g_pendingBuy_LowestAsk + retrace_points - eps)
      {
         if(InpVerbose)
            Print("Re-entry BUY triggered! | Lowest_Ask_Reached: ", DoubleToString(g_pendingBuy_LowestAsk, DigitsForSymbol()), 
                  " Current_Ask: ", DoubleToString(ask, DigitsForSymbol()));
         return true;
      }
      return false;
   }
   
   if(sideToOpen == POSITION_TYPE_SELL)
   {
      if(!g_pendingSell) return true;

      // Active Trade is BUY. It profits as price rises.
      // Track how high the Bid pushes upward.
      if(g_pendingSell_HighestBid <= 0.0 || bid > g_pendingSell_HighestBid)
         g_pendingSell_HighestBid = bid;

      // Rule: Re-enter SELL when price retraces DOWN by exactly 10 pips from highest point
      if(bid <= g_pendingSell_HighestBid - retrace_points + eps)
      {
         if(InpVerbose)
            Print("Re-entry SELL triggered! | Highest_Bid_Reached: ", DoubleToString(g_pendingSell_HighestBid, DigitsForSymbol()), 
                  " Current_Bid: ", DoubleToString(bid, DigitsForSymbol()));
         return true;
      }
      return false;
   }
...
bool ManageCloseMechanism()
{
   ...
   // This closing rule is intended for the hedged state: one BUY and one SELL.
...
int OnInit()
{
   ...
   Print("Close & Re-entry settings | digits=", DigitsForSymbol(),
         " pip_size=", DoubleToString(pip, DigitsForSymbol()),
         " target_close=", DoubleToString(InpCloseTargetPips * pip, DigitsForSymbol()),
         " re-entry_retrace=", DoubleToString(InpReentryRetracePips * pip, DigitsForSymbol()));
...
void OnTick()
{
   ...
   bool changed = ManageCloseMechanism();

   // Re-entry track and execute mechanism
   MaintainInventory();
