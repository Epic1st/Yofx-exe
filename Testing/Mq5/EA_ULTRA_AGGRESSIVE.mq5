...
input double   InpTrailingStop    = 50;     // Trailing Stop (pontos)
input double   InpTrailingStep    = 10;     // Trailing Step (pontos)
...
void OnTick()
{
   ...
   // Check Trailing Stop a cada tick
   CheckTrailingStop();
...
{
   if(!InpUseTrailingStop || InpTrailingStop <= 0) return;
   
   sym.Refresh();
   double point = sym.Point();
   int digits = (int)sym.Digits();
   double ts_points = InpTrailingStop * point;
   double step_points = InpTrailingStep * point;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(pos.SelectByIndex(i))
      {
         if(pos.Magic() == InpMagic && pos.Symbol() == _Symbol)
         {
            ...
            {
               if(current_price - open_price > ts_points)
               {
                  double new_sl = NormalizeDouble(current_price - ts_points, digits);
                  if(current_sl == 0 || new_sl - current_sl >= step_points)
                  {
                     trade.PositionModify(pos.Ticket(), new_sl, current_tp);
                  }
               }
            }
            else if(pos.PositionType() == POSITION_TYPE_SELL)
            {
               if(open_price - current_price > ts_points)
               {
                  double new_sl = NormalizeDouble(current_price + ts_points, digits);
                  if(current_sl == 0 || current_sl - new_sl >= step_points)
                  {
                     trade.PositionModify(pos.Ticket(), new_sl, current_tp);
                  }
               }
