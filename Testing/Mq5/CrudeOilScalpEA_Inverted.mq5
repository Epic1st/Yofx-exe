...
//|              v3.1 — INVERTED Entry Logic                         |
//|              BUY signal => opens SELL trade, SELL signal => BUY |
//|              Based on © nicks1008 Pine Script Indicator          |
//+------------------------------------------------------------------+
#property copyright "CrudeOilScalpEA v3.1 — Inverted Entry Edition"
...
int OnInit()
  {
   ...
   Print("CrudeOilScalpEA v3.1 [INVERTED] initialized. Magic=", InpMagicNumber);
...
void OnTick()
  {
   ...
   // NOTE: Logic is inverted — a BUY signal now opens a SELL, so we close BUY positions on a BUY signal
   if(CloseOnOppositeSignal)
     {
      if(anyBuy)  CloseTradesByDirection(ORDER_TYPE_BUY);   // BUY signal => closing existing BUYs before opening SELL
      if(anySell) CloseTradesByDirection(ORDER_TYPE_SELL);  // SELL signal => closing existing SELLs before opening BUY
     }

   // Open new trades — INVERTED: BUY signal => SELL trade, SELL signal => BUY trade
   if(anyBuy)  OpenTrades(ORDER_TYPE_SELL, sig.sl);
   if(anySell) OpenTrades(ORDER_TYPE_BUY,  sig.sl);
  }
...
SignalResult GetSignal()
  {
   ...
   // SL computation — INVERTED: BUY signal now opens SELL trade => swing HIGH SL
   //                             SELL signal now opens BUY trade => swing LOW SL
   if(isBUY || llrev_b)
     {
      // These signals now open SELL trades — SL above swing high
     ...
     {
      // These signals now open BUY trades — SL below swing low
...
void CreatePanel()
  {
   ...
   // Title
   CreateLabel(PFX+"title", x+10, y+7, "⚡  CRUDE OIL SCALPER  v3.1 [INV]", "Arial Bold", 10, clrWhite);
