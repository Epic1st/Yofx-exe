...
-- must provision the seven named roles before running it and keep runtime logins out
...
begin
    ...
    (
        ...
        where rolname in
            ('yo4x_control_api', 'yo4x_admin_bff', 'yo4x_emergency', 'yo4x_secret_ingestion', 'yo4x_conversion_worker', 'yo4x_worker')
...
begin
    execute format(
        ...
        current_database());
    execute format(
        'grant connect on database %I to yo4x_control_api, yo4x_admin_bff, yo4x_emergency, yo4x_secret_ingestion, yo4x_conversion_worker, yo4x_worker',
        current_database());
end
$$;
...
grant usage on schema identity, control, operations, audit, messaging
    to yo4x_secret_ingestion;
grant usage on schema identity, control, governance, audit, messaging
...
grant execute on function control.current_tenant_id(), control.current_actor_id(),
    ...
    to yo4x_control_api, yo4x_admin_bff, yo4x_emergency, yo4x_secret_ingestion,
       yo4x_conversion_worker, yo4x_worker;
...
grant execute on function control.acquire_u0_authority_lock(),
    ...
    to yo4x_control_api, yo4x_admin_bff, yo4x_emergency,
       yo4x_secret_ingestion, yo4x_worker;
grant execute on function control.acquire_u0_authority_lock()
    to yo4x_conversion_worker;
...
grant select (id, tenant_id, user_id, broker_id, broker_profile_id, server,
    ...
    row_version, created_at, updated_at)
    on operations.broker_accounts to yo4x_control_api;
grant insert on control.credential_ingestion_grants to yo4x_control_api;
grant insert (id, tenant_id, user_id, source_label, capability_sha256,
    state, expires_at, row_version, created_at, updated_at)
...
-- Secret ingestion: hash-only grant reservation/completion, opaque account
-- reference update, and redacted evidence. It has no access to any secret store.
grant select on control.credential_ingestion_grants to yo4x_secret_ingestion;
grant select (id, tenant_id, state, credential_state)
    on operations.broker_accounts to yo4x_secret_ingestion;
grant update (state, reservation_id, reserved_at, reservation_expires_at,
    cleanup_claim_token, cleanup_claimed_by, cleanup_claim_expires_at,
    completion_digest, consumed_at, row_version, updated_at)
    on control.credential_ingestion_grants to yo4x_secret_ingestion;
grant update (credential_reference, credential_state, row_version, updated_at)
    on operations.broker_accounts to yo4x_secret_ingestion;
grant insert on audit.audit_events, messaging.outbox_messages to yo4x_secret_ingestion;
...
grant execute on function control.acquire_strategy_import_job(uuid, bytea, uuid),
    control.acquire_strategy_import_persistence_lock(uuid),
    control.complete_strategy_import_job(uuid, uuid)
    to yo4x_conversion_worker;
...
grant select on operations.deployments, operations.worker_nodes, operations.worker_assignments,
    ...
    operations.deployment_reconciliations, control.command_targets,
...
grant insert on operations.worker_assignments, operations.execution_leases,
    operations.runtime_event_cursors, operations.runtime_event_inbox,
    operations.deployment_reconciliations, messaging.outbox_messages,
...
grant update (state, reservation_id, reserved_at, reservation_expires_at,
    ...
    row_version, updated_at)
    on control.credential_ingestion_grants to yo4x_worker;
grant update (credential_state, row_version, updated_at)
    on operations.broker_accounts to yo4x_worker;
grant update (observed_state, lease_expires_at, last_reconciled_at,
    row_version, updated_at)
    on operations.deployments to yo4x_worker;
grant update on messaging.outbox_messages, readmodel.deployment_health to yo4x_worker;
