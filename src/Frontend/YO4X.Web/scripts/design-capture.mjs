/*
 * Visual verification harness for the Bot Dashboard implementation.
 *
 * The ControlPlane API is NOT running here, so every /v1 route is intercepted and
 * answered with a representative payload that satisfies the real runtime decoders.
 * This proves layout, typography and colour against the approved canvas — it is a
 * rendering check, not evidence that any backend returned this data.
 */
...
const outputDirectory = resolve(process.env.YO4X_QA_OUTPUT ?? '.qa/design');
...
await context.addInitScript(() => {
  window.__YO4X_AUTH__ = {
    beginLogin: async () => {},
    getAccessToken: async () => 'design-capture-token',
  };
});
...
await context.route('**/v1/**', async (route) => {
  ...
  if (body === undefined) {
    await route.fulfill({ status: 404, contentType: 'application/problem+json', body: JSON.stringify({ type: 'about:blank', title: 'Not found', status: 404, code: 'RESOURCE_NOT_FOUND', correlationId: '0'.repeat(32) }) });
    return;
  }
...
if (preview) {
  ...
  console.log('Preview window open with the API stubbed. Close the window to end.');
  await page.waitForEvent('close', { timeout: 0 });
  await browser.close();
  process.exit(0);
}

const views = ['dashboard', 'strategies', `strategies/${id(11)}`, 'bots', 'backtests', 'cloud', 'journal', 'settings'];
for (const view of views) {
  await page.goto(`${appUrl}#${view}`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(450);
  const name = view.includes('/') ? 'strategy-detail' : view;
